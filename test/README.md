...
**Documentation**: Run `./test/yq-edge-cd-tests.sh ./posix-yq` for detailed test results
...
### Edge-CD Test Results (Current Status)

Run tests with: `./test/yq-edge-cd-tests.sh ./posix-yq`

**Passing Categories:**
- Basic file reading and key access
- Nested path navigation
- Array iteration with `.[]`

**In Progress:**
- Complex nested queries with multiple pipes
...
- `-r` flag (raw output)
- `-e` flag (exit on null)
- `-I` flag (indentation control)
- `-o=j` flag (JSON output shorthand)

**How to Track Progress:**
Run the edge-cd test suite after implementing features to see which tests now pass.
...
- `test/README.md` - This file (test suite overview)
- `test/QUICKSTART.md` - Quick start guide for edge-cd tests
...
3. Update test/README.md with current test status
4. Update README.md if adding new yq features
